"""Phase 5b — train InceptionTime with the same scaffold as CNN-BiLSTM.

Same 5-fold GKF + AMP + per-fold/per-epoch checkpointing as the rest of
the DL track. InceptionTime defaults: depth=6, n_filters=32, kernels [11,21,41].

Usage:
    python -m src.models.train_inception_time --gpu --name v1 [--epochs 40]
"""
...
def build_or_load_seq_cache():
    ...
    np.save(cx, Xtr); np.save(cxte, Xte); np.save(cy, ytr); np.save(cti, test_ids)
...
def train_one_fold(fold_k, Xtr, ytr, Xva, yva, args, class_w, device, ckpt_dir):
    ...
    model = InceptionTime(in_channels=6, n_classes=N_CLASSES, n_filters=args.n_filters, depth=args.depth).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(f"  Fold {fold_k} ep {epoch+1:>2d}/{args.epochs}: loss={train_loss:.4f}  val_f1={val_f1:.4f}  ({elapsed:.1f}s)", flush=True)
    ...
    print(f"  Fold {fold_k}: reloading best-val state (best={best_val_f1:.4f})")
...
def main():
    ...
    print(f"Run name: inception_time_{args.name}")
    print(f"Device: {device}  cuda available: {torch.cuda.is_available()}")
    print(f"Architecture: depth={args.depth}, n_filters={args.n_filters}")
    ...
    n = sum(p.numel() for p in InceptionTime(in_channels=6, n_classes=N_CLASSES, n_filters=args.n_filters, depth=args.depth).parameters())
    print(f"InceptionTime params: {n:,}")

    ckpt_dir = run_dir(f"inception_time_{args.name}")
    ...
    for k, (tr_idx, va_idx) in enumerate(folds):
        cached = load_fold(f"inception_time_{args.name}", k)
        ...
        save_fold(f"inception_time_{args.name}", k, preds_k, probs_k)
        f1_k = float(f1_score(ytr[va_idx], preds_k, average="macro"))
    ...
    np.save(ROOT / "oof" / f"inception_time_{args.name}_oof.npy", oof_probs)
    ...
    if final_ckpt.exists():
        ...
        for ep in range(epoch_budget):
            ...
            scheduler.step()
            print(f"  Final ep {ep+1}/{epoch_budget}: loss={total/len(full_ds):.4f}  ({time.time()-t0:.1f}s)", flush=True)
    ...
    np.save(ROOT / "oof" / f"inception_time_{args.name}_test_probs.npy", test_probs)
    sub_path = ROOT / "submissions" / f"sub_inception_time_{args.name}.csv"
    to_submission(test_ids, test_preds, str(sub_path))

    log_path = ROOT / "submissions" / "log.md"
    with open(log_path, "a", encoding="utf-8") as f:
        f.write(f"| {date.today().isoformat()} | sub_inception_time_{args.name} | "
                f"InceptionTime (depth={args.depth}, n_filters={args.n_filters}) | "
                f"{cv_mean:.4f} (fold-mean) / {oof_macro:.4f} (OOF) | _pending_ | _pending_ | "
                f"per-class F1 {[round(float(x), 4) for x in per_class_f1]} |\n")
    sidecar = {
        "model": f"inception_time_{args.name}",
        "depth": args.depth, "n_filters": args.n_filters,
        "epochs": args.epochs, "batch": args.batch, "lr": args.lr, "mixup_alpha": args.mixup_alpha,
    ...
    with open(ROOT / "oof" / f"inception_time_{args.name}_meta.json", "w", encoding="utf-8") as fh:
